...
def _transform_mode_label(mode: str) -> str:
    ...
    if mode == "ios-fix":
        return "ios-fix — mixed→socks, Автовыбор roundRobin без fallbackTag"
...
def _do_edit_transform_mode(repo: CatalogRepository) -> None:
    ...
    print_menu_item(3, "ios-fix — socks вместо mixed, Автовыбор roundRobin без fallbackTag")
...
def balancer_create(
    ...
    balancer_tag = tag.strip() if tag else suggest_balancer_tag(taken)
